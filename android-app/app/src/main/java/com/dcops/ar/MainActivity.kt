...
class MainActivity : AppCompatActivity() {
    ...
    // Debug: tap the screen to toggle running the bundled test image instead of the camera.
    ...
    private fun toggleTestMode() {
        testMode = !testMode
        if (testMode) {
            binding.statusText.text = "TEST MODE: dc_ops test image..."
            modelManager.processTestAsset(this, "test_rack.jpg") { results ->
                runOnUiThread {
                    renderTelemetry(results)
                    binding.statusText.text = "TEST: ${results.size} det - ${results.joinToString { it.label }}"
                }
            }
        } else {
            renderTelemetry(emptyList())
            binding.statusText.text = getString(
                if (modelReady) R.string.status_ready else R.string.status_booting
            )
        }
    }
